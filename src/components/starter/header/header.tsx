...
export default component$(() => {
  ...
  const fetchRandomPlugin = $(async (): Promise<void> => {
    ...
    try {
      ...
      if (!apiResponse.success) throw new Error('API responded with an unsuccessful status');
      store.plugin = apiResponse.data;
      console.log(store.plugin);
      window.location.href = `/${apiResponse.data.name}`;
    } catch (error: any) {
      store.error = error;
